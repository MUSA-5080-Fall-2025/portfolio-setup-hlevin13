...
census_api_key(Sys.getenv("5a82e243438bea307ae1c04f150d539c4db5fa47"))
...
population_vars <- acs_vars_2022 %>%
  filter(str_detect(label, "Total.*population")) # looking for total something population
...
income_vars <- acs_vars_2022 %>%
  filter(str_detect(label, "[Mm]edian.*income")) # Mm is a trick for capitalization - searches for all variations

# Look specifically for household income
household_income <- income_vars %>% # from those incomes found, look for household
...
# ``B01002_001`` - median age by sex across the US
...
# Case-insensitive search using regex - ignores the case
...
ggplot(county_reliability) +
  aes(x = pop_category, y = income_moe_pct, fill = pop_category) + # fill should vary based on category - randomly assigns
  geom_boxplot() +
  labs(
